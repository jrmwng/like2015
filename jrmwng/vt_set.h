...
namespace jrmwng
{
	...
	struct vt_set<T, tElement, vtSubSet...>
	{
		...
		struct exists
		{
			typedef typename internals::vt_set_exists<T, tTest, tElement, vtSubSet...>::type type;
		};
		template <T tTest>
		using exists_t = typename exists<tTest>::type;
		...
		struct unionset
		{
			typedef typename internals::vt_set_uniqueset<1 + sizeof...(vtSubSet) +sizeof...(vtThatSet), T, tElement, vtSubSet..., vtThatSet...>::type type;
		};
		template <T... vtThatSet>
		using unionset_t = typename unionset<vtThatSet...>::type;
		...
		struct intersectionset
		{
			typedef typename internals::vt_set_intersectionset<1 + sizeof...(vtSubSet), sizeof...(vtThatSet), T, tElement, vtSubSet..., vtThatSet...>::type type;
		};
		template <T... vtThatSet>
		using intersectionset_t = typename intersectionset<vtThatSet...>::type;

		static_assert(internals::vt_set_exists<T, tElement, vtSubSet...>::type::value == false, "");
	};
}